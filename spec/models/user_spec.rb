...
describe User do
	...
	describe "Cuando el formato del email es invalido" do
		it "debe der invalido" do
			direcciones= %w[user@prueba,com usuario_numero_.org usuario@prueba.comusuario@prueba.com usuario1@prueba+prueba,com]
	...
	describe "Cuando el email ya existe" do
		before do
			usiario_con_el_mismo_email = @user.dup
			usiario_con_el_mismo_email.email = @user.email.upcase
			usiario_con_el_mismo_email.save
		end 
		it { should_not be_valid}
	end
	...
	describe "devolver el valor del metodo de autentificacion" do
		before {@user.save}
